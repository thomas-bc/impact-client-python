...
def test_default_options(
    ...
    simulation_opts = simulation_options.with_defaults()
    runtime_opts = runtime_options.with_defaults()
    solver_opts = solver_options.with_defaults()
    compiler_opts = compiler_options.with_defaults()

    assert dict(compiler_opts) == {'c_compiler': 'msvs'}
    assert dict(runtime_opts) == {'log_level': 2}
    assert dict(simulation_opts) == {'ncp': 500}
    assert dict(solver_opts) == {'rtol': 1e-05}
