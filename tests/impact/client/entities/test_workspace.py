...
class TestWorkspace:
    ...
    def test_upload_dependency(self, workspace):
        workspace.entity.upload_dependency("c/test.mol")
        workspace.service.library_import.assert_called_with(
            "AwesomeWorkspace", "c/test.mol"
        )

    def test_upload_editable_dependency_raises_error(self, workspace):
        with pytest.raises(ValueError):
            workspace.entity.upload_dependency("c/test.mo")
    ...
    def test_get_enabled_dependencies(self, workspace):
        dependencies = workspace.entity.get_dependencies(include_disabled=False)
        assert dependencies == [
            create_project_entity(
                "cdbde8922bd2c48c392b1b4bb740adc0273c737c",
                definition={
